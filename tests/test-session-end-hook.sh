...
SESSION_END_HOOK="$PROJECT_ROOT/hooks/session-end.js"
...
OUTPUT=$(echo "$INPUT" | node "$SESSION_END_HOOK" 2>/dev/null)
...
OUTPUT2=$(echo "$INPUT2" | node "$SESSION_END_HOOK" 2>/dev/null)
...
OUTPUT3=$(echo "$INPUT3" | node "$SESSION_END_HOOK" 2>/dev/null)
